...
import { polishers, bagTypes, items } from '../../data/mockData';
...
export function CrudPage({ entityType, entityName, fields, validationSchema }: CrudPageProps) {
  ...
  const generateMockData = (type: string) => {
    ...
    switch (type) {
      case 'polisher':
        return polishers.map(polisher => ({
          ...baseData,
          id: polisher.id.toString(),
          name: polisher.name,
          code: `POL-${polisher.id.toString().padStart(3, '0')}`,
          status: 'active',
        }));
      case 'user':
        return [
      ...
      case 'bag-type':
        return bagTypes.map(bagType => ({
          ...baseData,
          id: bagType.id.toString(),
          name: bagType.type,
          code: bagType.type.split(' ').map(word => word[0]).join('').toUpperCase(),
          expectedWeight: bagType.weight,
          tolerance: bagType.weight * 0.1,
        }));
      case 'items':
        return items.map((item, index) => ({
          ...
          code: item.code,
          category: 'General',
          description: `Standard weight: ${item.standardWeight}kg`,
        }));
      default:
        return [];
    }
